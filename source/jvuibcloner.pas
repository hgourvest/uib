...
(* 
  ...
  with TJvUIBCloner.Create(nil) do
  begin
    OrgDatabase := JvUIBDbPleine;
    DestDatabasqe := JvUIBDbArchive;
...
uses
  ...
  SysUtils,
  jvuib, jvuiblib, jvuibase, jvuibmetadata;

type
  TOnProgressEvent = procedure(Sender: TObject; const TableName: String; Done: Integer) of object;

type
  TJvUIBCloner = class(TJvUIBComponent)
  private
    FOrgDB: TJvUIBDataBase;
    FDestDB: TJvUIBDataBase;
    FOnProgress: TOnProgressEvent;

    FDestMetaDB: TMetaDataBase;
    FOrgMetaDB: TMetaDataBase;

    function GetDBMetadatas(ADB: TJvUIBDatabase): TMetaDataBase;
  ...
  published
    property OrgDatabase: TJvUIBDatabase read FOrgDB write FOrgDB;
    property DestDatabase: TJvUIBDatabase read FDestDB write FDestDB;
...
destructor TJvUIBCloner.Destroy;
begin
...
procedure TJvUIBCloner.CloneTable(const TableName, FromSQL: String);
var
  OrgTable: TMetaTable;
  sql: String;
  i,f,d: Integer;
  ...
  RdrQuery: TJvUIBQuery;
  RdrTransaction: TJvUIBTransaction;
  WrtTransaction: TJvUIBTransaction;
begin
  { Objets transaction }
  RdrTransaction := TJvUIBTransaction.Create(nil);
  WrtTransaction := TJvUIBTransaction.Create(nil);
  { Requête qui lit dans la base d'origine }
  RdrQuery := TJvUIBQuery.Create(nil);
  try
...
procedure TJvUIBCloner.ActivateTriggers;
var
  T: TJvUIBTransaction;
  i,j: Integer;
...
procedure TJvUIBCloner.DeActivateTriggers;
var
  T: TJvUIBTransaction;
  i,j: Integer;
...
function TJvUIBCloner.GetDestMetadatas: TMetaDataBase;
begin
...
function TJvUIBCloner.GetOrgMetadatas: TMetaDataBase;
begin
...
function TJvUIBCloner.GetDBMetadatas(ADB: TJvUIBDatabase): TMetaDataBase;
var
  T: TJvUIBTransaction;
begin
  T := TJvUIBTransaction.Create(nil);
