...
type
  ...
  private
    FPosition: Smallint;
    FMechanism: Smallint;
    procedure LoadFromQuery(QField, QCharset, QArrayDim: TJvUIBStatement); override;
    procedure LoadFromStream(Stream: TStream); override;
  public
    class function NodeType: TMetaNodeType; override;
    procedure SaveToDDLNode(Stream: TStringStream); override;
  ...
  private
    ...
    FReturn: Smallint;
    procedure LoadFromStream(Stream: TStream); override;
    procedure LoadFromQuery(QNames, QFields, QCharset, QArrayDim: TJvUIBStatement; OIDs: TOIDUDFs);
...
begin
  ...
  Query := TJvUIBStatement.Create{$IFNDEF UIB_NO_COMPONENT}(nil){$ENDIF};
...
procedure TMetaBaseField.LoadFromQuery(QField, QCharset, QArrayDim: TJvUIBStatement);
...
var
  ...
  begin
    Q := TJvUIBStatement.Create{$IFNDEF UIB_NO_COMPONENT}(nil){$ENDIF};
...
procedure TMetaUDF.LoadFromQuery(QNames, QFields, QCharset, QArrayDim: TJvUIBStatement; OIDs: TOIDUDFs);
begin
  FName := Trim(QNames.Fields.AsString[0]);
...
procedure TMetaTableField.LoadFromQuery(Q, C, A: TJvUIBStatement);
var i: Integer;
begin
...
procedure TMetaUDFField.LoadFromQuery(QField, QCharset, QArrayDim: TJvUIBStatement);
begin
